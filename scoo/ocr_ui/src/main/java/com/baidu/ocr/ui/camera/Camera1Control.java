...
//            takingPicture.set(false);
    ...
    private void initCamera() {
        ...
        try {
            //官方的权限检测,但是对于国产手机并没有用,有些手机系统拒绝访问但是一样的能通过检测
            ...
            if (camera == null) {
                ...
                try {
                    ...
                    e.printStackTrace();
//                    startPreview(true);
                    //对于大部分国产手机,假如系统拍照权限拒绝访问,这里会报错
//                    showGotoSettingDialog();
                    permissionCallback.showGotoSettingDialog();
                    return;
                }
            ...
            if (Build.BRAND.equalsIgnoreCase("meizu")) {
                Toast.makeText(context, R.string.camera_permission_required, Toast.LENGTH_LONG)
                        .show();
            }
        }
    }
                             ...
                             try {
                                 camera.autoFocus(new Camera.AutoFocusCallback() {
                                     ...
                                     public void onAutoFocus(boolean success, Camera camera) {
                                         parameters = camera.getParameters();//处理魅族机型获取再自动对焦
                                         parameters.setFocusMode(Camera.Parameters.FOCUS_MODE_AUTO);
                                         camera.setParameters(parameters);
                                     }
                                 });
