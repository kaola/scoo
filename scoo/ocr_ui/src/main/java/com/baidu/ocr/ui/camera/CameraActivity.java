...
public class CameraActivity extends Activity {
    ...
    private PermissionCallback permissionCallback = new PermissionCallback() {
        ...
        public void showGotoSettingDialog() {
            showGotoSetting(CameraActivity.this);
        }
    };
    public static TCAgentAlbumBack album1;

    public interface TCAgentAlbumBack {
        void album(boolean flag);
    }

    public static void setAlbum(TCAgentAlbumBack album) {
        album1 = album;
    }

    public static TCAgentConfirmBack confirm1;

    public interface TCAgentConfirmBack {
        void confirm();
    }

    public static void setConfirm(TCAgentConfirmBack confirm) {
        confirm1 = confirm;
    }
            ...
            case PERMISSIONS_REQUEST_CAMERA: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(getApplicationContext(), R.string.camera_permission_required, Toast.LENGTH_LONG)
                            .show();
                    //add by zyt : 如果用户直接禁止掉拍照权限,则不进入黑屏界面
                    finish();
                }
                break;
            }
     ...
     * 去设置对话框
     */
    public static   Dialog showGotoSetting(final Activity context) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        View view = ((Activity) context).getLayoutInflater().inflate(R.layout.dialog_permission, null);
        TextView tv_dialog_home_page_title = (TextView) view.findViewById(R.id.tv_dialog_home_page_title);
        Button tvSecTitle = (Button) view.findViewById(R.id.btn_home_page_dialog_sure);
        ImageView iv_close_set = (ImageView) view.findViewById(R.id.iv_dialog_home_page_x);
        RelativeLayout relativeBg = (RelativeLayout) view.findViewById(R.id.bg_relative);
        relativeBg.setBackgroundColor(context.getResources().getColor(android.R.color.black));

        tvSecTitle.setText("确定");
        tv_dialog_home_page_title.setText("您需要打开相机权限才可以继续,很抱歉");
        ...
        tvSecTitle.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                dialog.dismiss();
